...
export default {
  ...
  methods: {
    syncDealedOrders() {
      var syncOrderInfo = localStorage.getItem("SyncOrderInfo");
      if (syncOrderInfo == null) {
        syncOrderInfo = {lastIndex: 0, orderList: []};
      } else {
        syncOrderInfo = JSON.parse(syncOrderInfo)[this.chainId];  // {lastIndex, orderList}
        syncOrderInfo.lastIndex = parseInt(syncOrderInfo.lastIndex);
      }
      this.curPairContract.methods.getOrderNumber(2).call().then(dealedOrderNumber => {
        var fromIndex = syncOrderInfo.lastIndex;
        const allPromise = [];
        for (var i = fromIndex; i < dealedOrderNumber; i += this.pageSize) {
          const onePromise = this.boboPairHelper.methods.getOneStatusOrders(this.pairInfo.pairAddr, 2, i, this.pageSize).call();
          allPromise.push(onePromise);
        }
        Promise.all(allPromise).then(ordersList => {
          syncOrderInfo.orderList.map(orderInfo => {
            this.processOrder(orderInfo);
          });
          ordersList.map(orders => {
            console.log(orders);
            syncOrderInfo.lastIndex += parseInt(orders.count);
            orders.nftInfos.map(nftInfo => {
              var nftObj = {time: nftInfo.dealedTime, bBuyQuoteToken: nftInfo.bBuyQuoteToken, price: nftInfo.spotPrice, 
                            volumn: nftInfo.bBuyQuoteToken ? nftInfo.inAmount : nftInfo.outAmount};
              syncOrderInfo.orderList.push(nftObj);
              this.processOrder(nftObj);
            });
          });
          console.log("orderAggregator", this.orderAggregator);
          const orderAggregator = [];
          Object.keys(this.orderAggregator).map(time => {
            const item = [];
            item.push(this.orderAggregator[time].time);
            item.push(this.orderAggregator[time].open);
            item.push(this.orderAggregator[time].close);
            item.push(this.orderAggregator[time].low);
            item.push(this.orderAggregator[time].high);
            item.push(this.orderAggregator[time].volumn);
            orderAggregator.push(item);
          });
          this.orderAggregator = orderAggregator;
          console.log(this.orderAggregator);
          this.drawLine();
          const ordersListInfo = {};
          ordersListInfo[this.chainId] = syncOrderInfo;
          localStorage.setItem("SyncOrderInfo", JSON.stringify(ordersListInfo));
        });
      });
    },
    processOrder(orderInfo) {
      //console.log('processOrder', orderInfo);
      const time = this.getValidTime(orderInfo.time);
      const price = this.getReadableNumber(orderInfo.price, 6, 2);
      const volumn = this.getReadableNumber(orderInfo.volumn, 6, 2);
      if (this.orderAggregator[time] == null) {  // [time:"2004-01-02",open:10452.74,close:10409.85,low:10367.41,high:10554.96,volumn:168890000] 
        this.orderAggregator[time] = {time, open: price, close: price, low: price, high:price,volumn};
      } else {
        this.orderAggregator[time].close = price;
        this.orderAggregator[time].low = price < this.orderAggregator[time].low ? price : this.orderAggregator[time].low;
        this.orderAggregator[time].high = price > this.orderAggregator[time].high ? price : this.orderAggregator[time].high;
        this.orderAggregator[time].volumn += volumn;
      }
    },
    getReadableNumber(value, assetDecimal, displayDecimal) {
      ...
      renderValue = renderValue.toNumber();
      return renderValue;
    },
    getValidTime(timestamp) {
      ...
      return date.format("yyyy-MM-dd hh:00:00"); // +  '.' + (date.getMilliseconds() + 1000 + '').substr(1);
    },
    closeMsg(ev) {
    ...
    drawLine() {
      ...
      var data = this.splitData(this.orderAggregator);
      myChart.setOption(
        (option = {
          animation: false,
    ...
    splitData(rawData) {
      ...
      var values = [];
      var volumes = [];
      for (var i = 0; i < rawData.length; i++) {    //  ["2016-06-22",17832.67,17780.83,17770.36,17920.16,89440000]
        categoryData.push(rawData[i].splice(0, 1)[0]);  // 日期 "2016-06-22"
        values.push(rawData[i]);    // [17832.67,17780.83,17770.36,17920.16,89440000]
        volumes.push([
          i,
          rawData[i][4],  // 交易量
          rawData[i][0] > rawData[i][1] ? 1 : -1,  // -1: 跌，1：涨
        ]);
      }
