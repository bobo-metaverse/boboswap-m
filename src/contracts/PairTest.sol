...
contract BoboPairTest is Ownable, OrderStore {
    ...
    address public constant OrderNFTAddr = 0x04673384398379a84d05895456AE02fc05B4a509;
    address public constant OrderDetailNFTAddr = 0x75508A1de2183a1F2Bf1822d6794698c953A1788;
    ...
    function addLimitedOrder(bool _bBuyQuoteToken, uint256 _spotPrice, uint256 _amountIn, uint256 _slippagePercent, 
                            bool _orderNFT, bool _boboFarmer, bool _exManager, bool _boboRouter) public {
        require(_slippagePercent <= 1000, "BoboPair: slippage MUST <= 1000(10%)");
        
        uint256 minOutAmount = 0;
        if (_slippagePercent >= 100) {
            if (_bBuyQuoteToken) {
        ...
        uint256 orderId = _orderNFT ? addOrder(_bBuyQuoteToken, _spotPrice, _amountIn, minOutAmount) : 1;
        ...
        ResultInfo memory bestSwapInfo;
        if (_boboRouter)
            (, bestSwapInfo) = boboRouter.getBestSwapPath(inToken, outToken, _amountIn);
