...
contract StratMaticSushi is Ownable, ReentrancyGuard, Pausable {
    ...
    function addLiquidAndFarm(address _tokenA, address _tokenB) internal {
        ...
        if (tokenABalance > minFarmAmount && tokenBBalance > minFarmAmount && curPidMap[_tokenA][_tokenB] > 0) {
            ...
            wantLockedTotalMap[_tokenA] = wantLockedTotalMap[_tokenA].add(tokenABalance);
            wantLockedTotalMap[_tokenB] = wantLockedTotalMap[_tokenB].add(tokenBBalance);
    ...
    function setControllerFee(uint256 _controllerFee) public onlyGov {
        require(_controllerFee <= devFundFeeUL, "too high");
        devFundFee = _controllerFee;
    }

    function setBurnedBOTRate(uint256 _burnedBOBORate) public onlyGov {
