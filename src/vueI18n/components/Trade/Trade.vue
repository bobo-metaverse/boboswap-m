      ...
      if (this.buy_sell) {   // 买单
        if (!this.baseTokenApproved) {  // 授权
          ...
          if (this.delegateType == 'limitedDelegate') {  // 限价买单
            ...
            const minOutAmount = '0x' + new BigNumber(inTokenAmount / pairInfo.currentPrice * (100 - this.slippagePercent) / 100).shiftedBy(this.pairInfo.decimals).toString(16);
        ...
        if (!this.quoteTokenApproved) {  // 授权
          ...
          if (this.delegateType == 'limitedDelegate') {  // 限价单
            ...
            const minOutAmount = '0x' + new BigNumber(inTokenAmount * pairInfo.currentPrice * (100 - this.slippagePercent) / 100).shiftedBy(6).toString(16);
            this.curStakeId = this.curPairContract.methods['addMarketOrder'].cacheSend(
                                                                                    true,
                                                                                    '0x' + new BigNumber(this.inTokenAmount).shiftedBy(6).toString(16),
                                                                                    minOutAmount,
